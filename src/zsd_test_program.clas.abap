CLASS zsd_test_program DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS ZSD_TEST_PROGRAM IMPLEMENTATION.
...
*      ls_product-vbeln        = |1000{ sy-index }|.
...
*    out->write( |100 kayıt tabloya eklendi| ).
  ...
  " CDS'ten veri çek
    SELECT * FROM zcds_product_sanshine
      INTO TABLE @DATA(lt_product).

    " Eğer tablo boşsa uyarı
    IF lt_product IS INITIAL.
      out->write( 'ZCDS_PRODUCT_SANSHINE tablosunda veri yok' ).
      RETURN.
    ENDIF.

    " Döngü ile ekrana yazdır
    LOOP AT lt_product INTO DATA(ls_product).
      out->write(
        |Sales Document: { ls_product-salesdocument } |
        && |Item: { ls_product-salesdocumentitem } |
        && |Customer: { ls_product-customer }|
      ).
    ENDLOOP.
